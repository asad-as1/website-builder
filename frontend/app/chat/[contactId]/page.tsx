...
export default function ChatRoomPage() {
  ...
  useEffect(() => {
    ...
    const handleDelivered = ({ contactId }: any) => {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.sender === "user" && (msg.status === "sent" || msg.status === "sending")
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] text-white flex flex-col relative overflow-hidden mt-8">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto pt-36 pb-32 px-4">
        <div className="max-w-3xl mx-auto space-y-3">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center text-center gap-2 py-24 text-gray-500">
              ...
              <p className="text-xs text-gray-600">Send a message and our team will reply here</p>
            </div>
          )}
          {messages.map((msg, i) => {
              ...
              <div key={msg.clientId || i} className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
                ...
                >
                  ...
                  >
                    ...
                    {isUser && <MessageTicks status={msg.status || "sent"} />}
                  </div>
                </div>
              </div>
            );
