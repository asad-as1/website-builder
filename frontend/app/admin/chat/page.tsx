...
export default function AdminChatListPage() {
  ...
  useEffect(() => {
    ...
    const handleMessagesRead = ({ contactId, readBy }: any) => {
      if (readBy === "admin") {
        setContacts((prev) =>
          prev.map((c) => (c.id === contactId ? { ...c, unreadByAdmin: 0 } : c))
        );
      }
    };
  ...
  const filteredContacts = useMemo(() => {
    ...
    if (filter === "read") list = list.filter((c) => (c.unreadByAdmin || 0) === 0);
  ...
  const totalUnread = contacts.reduce((sum, c) => sum + (c.unreadByAdmin || 0), 0);
